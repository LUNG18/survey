package com.gobestsoft.survey.util;
...
//	}
    ...
    public static List<Map<String, Object>> removeRepeatMapByKey(List<Map<String, Object>> list, String mapKey){
        ...
        //把list中的数据转换成msp,去掉同一id值多余数据，保留查找到第一个mapKey值对应的数据
        List<Map<String, Object>> listMap = new ArrayList<>();
        Map<Object, Map> msp = new HashMap<>();
        for(int i = list.size()-1 ; i>=0; i--){
            Map map = list.get(i);
            Object id = map.get(mapKey);
            map.remove(mapKey);
            msp.put(id, map);
        }
        //把msp再转换成list,就会得到根据某一字段去掉重复的数据的List<Map>
        Set<Object> mspKey = msp.keySet();
        for(Object key: mspKey){
